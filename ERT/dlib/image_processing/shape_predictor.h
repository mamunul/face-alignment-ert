...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        void set_rect(rectangle newRect) {
            if (prevRect.width() == 0) {
                prevRect = newRect;
            } else {
                const bool widthChanged = fabs((newRect.width() - prevRect.width())
                                               / (float) (prevRect.width())) > 0.2;
                const long prevRectCenterX = prevRect.left() + prevRect.width() / 2;
                const long prevRectCenterY = prevRect.top() + prevRect.height() / 2;
                const long newRectCenterX = newRect.left() + newRect.width() / 2;
                const long newRectCenterY = newRect.top() + newRect.height() / 2;
                const bool centerDisplaced = (sqrtf(powf(newRectCenterX - prevRectCenterX, 2)
                                                    + powf(newRectCenterY - prevRectCenterY, 2))
                                              / prevRect.width()) > 0.12;
                if (widthChanged || centerDisplaced) {
                    prevRect = newRect;
                }
            }
        }
